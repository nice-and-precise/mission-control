...
import type { WorkflowTemplate, WorkflowStage, SuggestedPlanningAgent } from '@/lib/types';
...
export function TeamTab({ taskId, workspaceId }: TeamTabProps) {
  ...
  const [suggestedAgents, setSuggestedAgents] = useState<SuggestedPlanningAgent[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [rolesRes, workflowsRes, taskRes, planningRes] = await Promise.all([
          fetch(`/api/tasks/${taskId}/roles`),
          fetch(`/api/workspaces/${workspaceId}/workflows`),
          fetch(`/api/tasks/${taskId}`),
          fetch(`/api/tasks/${taskId}/planning`),
        ]);
        ...
        if (planningRes.ok) {
          ...
          setSuggestedAgents(Array.isArray(planning.agents) ? planning.agents : []);
          setPlanningApproved(Boolean(planning.isApproved));
        }
      } catch (err) {
    ...
    <div className="space-y-6">
      <div className="rounded-lg border border-mc-border bg-mc-bg p-4">
        ...
        <p className="mt-2 text-sm text-mc-text-secondary">
          This section controls the actual task roles used during execution. Planner-suggested agents are shown separately below and are not auto-assigned to the running team.
        </p>
        <div className="mt-3 flex flex-wrap gap-2 text-xs text-mc-text-secondary">
          <span className="rounded-full border border-mc-border px-2 py-1">Task status: {taskStatus || 'unknown'}</span>
          <span className={`rounded-full border px-2 py-1 ${planningApproved ? 'border-green-500/30 text-green-300' : 'border-amber-500/30 text-amber-300'}`}>
            {planningApproved ? 'Planning approved' : 'Planning not approved'}
          </span>
          <span className="rounded-full border border-mc-border px-2 py-1">Assigned roles: {executionAssignments.length}</span>
        </div>
      </div>
      ...
      {/* Role Assignments */}
      <div>
        <label className="block text-sm font-medium mb-2">Execution Role Assignments</label>
        ...
        <div>
          ...
          <div className="space-y-2">
            {suggestedAgents.map((agent, index) => (
              <div key={`${agent.agent_id || agent.name}-${index}`} className="rounded-lg border border-mc-border bg-mc-bg p-3">
