...
import { getAgentLiveEmptyState, type AgentLiveStatus } from '@/lib/agent-live';
...
export function AgentLiveTab({ taskId }: AgentLiveTabProps) {
  ...
  const [status, setStatus] = useState<AgentLiveStatus>('connecting');
  ...
  const emptyState = getAgentLiveEmptyState(status, messages.length, activeStreams.length);
  ...
  return (
    <div className="flex flex-col h-full min-h-[400px]">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        {emptyState === 'session_ended' && (
          <div className="text-center py-12">
            <AlertCircle className="w-8 h-8 text-amber-400 mx-auto mb-3 opacity-80" />
            <p className="text-amber-400 text-sm">The latest agent session ended</p>
            <p className="text-mc-text-secondary/70 text-xs mt-1">
              No live transcript was captured for that run. Dispatch again to attach to the next session.
            </p>
          </div>
        )}
