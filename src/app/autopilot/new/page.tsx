...
export default function NewProductPage() {
  ...
  const [form, setForm] = useState({
    ...
    default_branch: 'main',
    workspace_mode: 'dedicated' as 'dedicated' | 'existing',
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch('/api/products', {
        ...
        body: JSON.stringify({
          ...form,
          workspace_id: form.workspace_mode === 'existing' ? form.workspace_id : undefined,
        }),
      });
      if (res.ok) {
      ...
      <main className="max-w-2xl mx-auto px-4 py-8">
        ...
        {step === 'basics' && (
          <div className="space-y-6">
            ...
            <div className="space-y-3 rounded-xl border border-mc-border bg-mc-bg-secondary p-4">
              <div>
                <h3 className="text-sm font-medium text-mc-text">Workspace destination</h3>
                <p className="mt-1 text-sm text-mc-text-secondary">
                  Autopilot build cards route into this workspace because approved ideas create tasks using the product&apos;s workspace.
                </p>
              </div>
              <div className="grid gap-3 md:grid-cols-2">
                <button
                  type="button"
                  onClick={() => setForm(f => ({ ...f, workspace_mode: 'dedicated' }))}
                  className={`rounded-lg border px-4 py-3 text-left transition-colors ${
                    form.workspace_mode === 'dedicated'
                      ? 'border-mc-accent bg-mc-accent/10'
                      : 'border-mc-border bg-mc-bg-tertiary hover:border-mc-accent/50'
                  }`}
                >
                  <div className="text-sm font-medium text-mc-text">Dedicated workspace</div>
                  <p className="mt-1 text-xs text-mc-text-secondary">
                    Recommended. Mission Control creates a product-specific workspace so build cards stay out of `default`.
                  </p>
                </button>
                <button
                  type="button"
                  onClick={() => setForm(f => ({ ...f, workspace_mode: 'existing' }))}
                  className={`rounded-lg border px-4 py-3 text-left transition-colors ${
                    form.workspace_mode === 'existing'
                      ? 'border-mc-accent bg-mc-accent/10'
                      : 'border-mc-border bg-mc-bg-tertiary hover:border-mc-accent/50'
                  }`}
                >
                  <div className="text-sm font-medium text-mc-text">Use existing workspace</div>
                  <p className="mt-1 text-xs text-mc-text-secondary">
                    Keep this product inside an existing queue, such as `default`, instead of creating a dedicated one.
                  </p>
                </button>
              </div>
              {form.workspace_mode === 'existing' && (
                <div>
                  <label className="block text-sm font-medium text-mc-text mb-2">Existing workspace</label>
                  <select
                  ...
                  >
                    {workspaces.map(workspace => (
                      <option key={workspace.id} value={workspace.id}>
                        {workspace.icon} {workspace.name} ({workspace.slug})
                      </option>
                    ))}
                  </select>
                </div>
            ...
            <button
              ...
              disabled={!form.name.trim() || (form.workspace_mode === 'existing' && !form.workspace_id) || saving}
