...
import { normalizeGatewayAgentModel } from '@/lib/openclaw/gateway-agents';
  ...
  try {
    ...
    transaction(() => {
      ...
      for (const agentReq of body.agents) {
        ...
        const normalizedModel = normalizeGatewayAgentModel(agentReq.model);

        if (normalizedModel.warning) {
          console.warn('[agents/import] Unsupported gateway agent model shape:', {
            gatewayId: agentReq.gateway_agent_id,
            warning: normalizedModel.warning,
            model: normalizedModel.rawModelConfig ?? (agentReq.model ?? null),
          });
        }
