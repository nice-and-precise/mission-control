...
async function runAIDecomposition(task: Task): Promise<{
  ...
  while (Date.now() - startTime < DECOMPOSE_TIMEOUT_MS) {
    ...
    const { messages, transcriptIssue } = await getMessagesFromOpenClaw(sessionKey);
    if (transcriptIssue) {
      throw new Error(`AI decomposition transcript could not be recovered: ${transcriptIssue.message}`);
    }
