...
export async function PATCH(
  ...
  try {
    ...
    const planningSpecLocked = !!queryOne<{ id: string }>(
      'SELECT id FROM planning_specs WHERE task_id = ? LIMIT 1',
      [id]
    );
    ...
    // may be approved by the assigned reviewer or a master agent.
    if (validatedData.status === 'done' && validatedData.updated_by_agent_id) {
      const updatingAgent = queryOne<Pick<Agent, 'id' | 'role' | 'is_master'>>(
        'SELECT id, role, is_master FROM agents WHERE id = ?',
        [validatedData.updated_by_agent_id]
      );
      const approvalPolicy = evaluateAgentApprovalPolicy({
      ...
      if (!approvalPolicy.allowed) {
        return NextResponse.json(
          { error: approvalPolicy.error || 'Forbidden: agent cannot approve this task transition' },
          { status: 403 }
        );
      }
    }
    ...
    } else if (existing.status === 'planning' && !planningSpecLocked) {
      readinessIssues.push('Planning not approved');
    }
    ...
    if (nextStatus !== undefined && nextStatus !== existing.status) {
      if (existing.status === 'planning' && ['assigned', 'in_progress', 'testing', 'review', 'verification', 'done'].includes(nextStatus) && !planningSpecLocked) {
        return NextResponse.json(
          { error: 'Planning must be approved before execution can begin' },
          { status: 400 }
        );
      }
    ...
    if (Object.prototype.hasOwnProperty.call(validatedData, 'pr_url')) {
      syncTaskPrDeliverable(id, validatedData.pr_url ?? null);
    }
