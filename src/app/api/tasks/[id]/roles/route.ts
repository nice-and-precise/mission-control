...
export async function PUT(
  ...
  try {
    ...
    const requestedAgentIds = Array.from(new Set(deduped.values()));
    if (requestedAgentIds.length > 0) {
      const placeholders = requestedAgentIds.map(() => '?').join(', ');
      const allowedAgents = queryAll<{ id: string }>(
        `SELECT id
         FROM agents
         WHERE workspace_id = ?
           AND COALESCE(scope, 'workspace') = 'workspace'
           AND id IN (${placeholders})`,
        [task.workspace_id, ...requestedAgentIds]
      );
      const allowedIds = new Set(allowedAgents.map((agent) => agent.id));
      const invalidAgentId = requestedAgentIds.find((agentId) => !allowedIds.has(agentId));
      if (invalidAgentId) {
        return NextResponse.json(
          { error: 'Task roles can only be assigned to workspace-scoped agents' },
          { status: 400 }
        );
      }
    }
