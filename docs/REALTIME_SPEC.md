...
const task = await fetch('http://YOUR_SERVER_IP:3000/api/tasks?status=inbox').then(r => r.json());

// 2. Log triage activity
await fetch(`http://YOUR_SERVER_IP:3000/api/tasks/${task.id}/activities`, {
  method: 'POST',
  body: JSON.stringify({
...
// 3. Update status
await fetch(`http://YOUR_SERVER_IP:3000/api/tasks/${task.id}`, {
  method: 'PATCH',
...
await fetch(`http://YOUR_SERVER_IP:3000/api/tasks/${task.id}/subagent`, {
  method: 'POST',
  body: JSON.stringify({
...
await fetch(`http://YOUR_SERVER_IP:3000/api/tasks/${task.id}/deliverables`, {
  method: 'POST',
  body: JSON.stringify({
