...
- it sends one automatic structured repair prompt on the same planning session before surfacing manual recovery
- if the planner still fails after that retry, it shows an explicit recovery message instead of pretending the old question is still active

**Recovery options:**
1. Wait for one more poll cycle after the bad planner reply; Mission Control now attempts one automatic repair.
2. If the planner is still non-JSON after that automatic retry, cancel and restart planning for that task.
...
- OpenCode Go remains discoverable in the catalog but is not the intended default live lane on this machine
